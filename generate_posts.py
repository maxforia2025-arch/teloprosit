#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""🫀 АВТОПОПОЛНЕНИЕ БАНКА канала «Тело просит» через Claude API.
...
def build_prompt(cfg, existing, n):
    ...
    pillars = cfg.get("pillars_hint", "Сигнал, Сон и ритм, Простое действие, Красный флаг")
    return (
        "Ты — редактор Telegram-канала «Тело просит».\n"
        "Ниша канала: Что тело пытается сказать сигналами, которые все игнорируют\n"
        "Тон: спокойный, заботливый, без запугивания и без диагнозов\n"
        ...
        "5. Обращение к читателю — безличное или на «вы», НИКОГДА на «ты»:\n"
        "   канал говорит о здоровье и панибратство здесь снижает доверие.\n"
        "6. Никаких диагнозов, препаратов, дозировок и БАД. Формулировки «это может значить»,\n"
        "   «стоит попробовать». Посты рубрики «Красный флаг» обязаны прямо вести к врачу.\n"
        "7. Объём текста — 5–8 предложений: пост должен успеть объяснить, а не только заявить.\n\n"
        ...
        "- text: разбор в 5–8 предложениях простым языком: что за сигналом стоит,\n"
        "  что можно попробовать сегодня и когда это уже вопрос к врачу\n"
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Автопополнение банка «Тело просит»")
