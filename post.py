#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""🫀 ПОСТЕР канала «Тело просит» — берёт следующий пост из банка и шлёт в Telegram.
...
который лежит в .gitignore). В коде секретов нет и быть не должно.
    BOT_TOKEN=123:ABC   CHANNEL_ID=@telo_prosit_daily   [FUNNEL_BOT_USERNAME=...]
"""
...
UA = "Mozilla/5.0 (telo_prosit-bot)"
...
def format_post(post, cfg):
    ...
    name = html.escape(str(cfg.get("channel_name", "Тело просит")))
    handle = html.escape(str(cfg.get("channel_handle", "@telo_prosit_daily")))
    ...
    if funnel:
        link = "https://t.me/" + funnel + "?start=" + str(cfg.get("src", "tpr")) + "_tg"
    ...
    # Редстандарт канала (CHANNEL_BIBLE §2): дисклеймер обязателен в КАЖДОМ посте.
    disclaimer = str(cfg.get("disclaimer", "")).strip()
    if disclaimer:
        parts.append("")
        parts.append("<i>" + html.escape(disclaimer) + "</i>")
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Постер канала «Тело просит»")
